    ...
    internal sealed class DotCompilerWorker : IDisposable
    {
        ...
        private void CompileAttributes(IReadOnlyList<IDotAttribute> attributes)
        {
            ...
            foreach (var attribute in attributes)
            {
                ...
                {
                    attributeValues.Add($"pos=\"{positionAttribute.Position.X},{positionAttribute.Position.Y},{positionAttribute.Position.Z}!\"");
                }
                else if (attribute is DotRotationAttribute rotationAttribute && rotationAttribute.Rotation != null)
                {
                    attributeValues.Add($"rot=\"{rotationAttribute.Rotation.X},{rotationAttribute.Rotation.Y},{rotationAttribute.Rotation.Z}!\"");
                }
                else if (attribute is DotCustomAttribute customAttribute)
                {
